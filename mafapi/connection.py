...
class Connection:
    ...
    async def __init__(self, pointer, roomid, session):
        ...
        self.info = await self.ws.receive_json()
        self.options = next(filter(lambda x:x['type']=='options', self.info['events'][::-1]))
        self.session = session
        self.roomid = roomid
        self.users = [x['userId'] for x in self.info['users']]
        for packet in self.events:
            await self.parse(packet, False)
    async def get(self):
    ...
    async def kick(self, user):
        ...
        if type(user)==dict:
            ...
            userid = user.id
        return await self.session.session.post('https://mafia.gg/api/rooms/{}/kick'.format(self.roomid), json={'userId': userid})
    async def shadowban(self, user):
        return await self.session.blacklist(user)
    async def ban(self, user):
        if type(user)==dict:
            return [await self.shadowban(user['username']), await self.kick(user['id'])]
        return [await self.shadowban(user), await self.kick(user)]
    async def parse(self, packet, append=True):
        if append:
            logging.debug('-> %r', packet)
